...
const PHRASES = [
    "🐕 {s} is so happy their tail is wagging at lightspeed!",
    "✨ {s} wags their imaginary tail in sheer excitement!",
    "🐾 {s} is overflowing with joy! *Wag wag wag*"
];
...
module.exports = {
    ...
    data: new SlashCommandBuilder()
        .setName('wag')
        .setDescription('Express yourself! Usage: -wag')
    ...
    async execute(interaction, client, args) {
        ...
        try {
            const res = await axios.get('https://nekos.best/api/v2/wag');
            const gif = res.data.results[0];

            const container = new ContainerBuilder()
                .setAccentColor(0xff6b8a)
                .addTextDisplayComponents(
                    new TextDisplayBuilder().setContent('# 🐕 Wag!')
                )
