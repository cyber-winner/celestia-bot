...
const PHRASES = [
    "🫨 {s} is shaking violently. Is it cold, or fear?!",
    "✨ {s} trembles nervously...",
    "🥶 {s} shivers and shakes uncontrollably."
];
...
module.exports = {
    ...
    data: new SlashCommandBuilder()
        .setName('shake')
        .setDescription('Express yourself! Usage: -shake')
    ...
    async execute(interaction, client, args) {
        ...
        try {
            const res = await axios.get('https://nekos.best/api/v2/shake');
            const gif = res.data.results[0];

            const container = new ContainerBuilder()
                .setAccentColor(0xff6b8a)
                .addTextDisplayComponents(
                    new TextDisplayBuilder().setContent('# 🤝 Shake!')
                )
