...
const PHRASES = [
    "😡 {s} is absolutely furious! Someone is going to pay!",
    "💢 {s} emits a terrifying, dark aura of pure anger.",
    "🤬 {s} is raging! Better stay out of their way!"
];
...
module.exports = {
    ...
    data: new SlashCommandBuilder()
        .setName('angry')
        .setDescription('Express yourself! Usage: -angry')
    ...
    async execute(interaction, client, args) {
        ...
        try {
            const res = await axios.get('https://nekos.best/api/v2/angry');
            const gif = res.data.results[0];

            const container = new ContainerBuilder()
                .setAccentColor(0xff6b8a)
                .addTextDisplayComponents(
                    new TextDisplayBuilder().setContent('# 😡 Angry!')
                )
