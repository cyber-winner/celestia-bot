...
const PHRASES = [
    "🌀 {s} spins around like a beautiful ballerina!",
    "🚁 {s} spins faster and faster... they're taking off!",
    "😵‍💫 {s} spun too much and is now completely dizzy."
];
...
module.exports = {
    ...
    data: new SlashCommandBuilder()
        .setName('spin')
        .setDescription('Express yourself! Usage: -spin')
    ...
    async execute(interaction, client, args) {
        ...
        try {
            const res = await axios.get('https://nekos.best/api/v2/spin');
            const gif = res.data.results[0];

            const container = new ContainerBuilder()
                .setAccentColor(0xff6b8a)
                .addTextDisplayComponents(
                    new TextDisplayBuilder().setContent('# 🌀 Spin!')
                )
