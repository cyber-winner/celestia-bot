...
const PHRASES = [
    "👫 {s} securely locks fingers with {t}. How lewd!",
    "✨ {s} blushes as they gently hold {t}'s hand.",
    "💖 {s} and {t} walk together, holding hands warmly."
];
...
module.exports = {
    ...
    data: new SlashCommandBuilder()
        .setName('handhold')
        .setDescription('Use handhold on someone! Usage: -handhold @user')
    ...
    async execute(interaction, client, args) {
        ...
        try {
            const res = await axios.get('https://nekos.best/api/v2/handhold');
            const gif = res.data.results[0];

            const container = new ContainerBuilder()
                .setAccentColor(0xff6b8a)
                .addTextDisplayComponents(
                    new TextDisplayBuilder().setContent('# 🤝 Handhold!')
                )
