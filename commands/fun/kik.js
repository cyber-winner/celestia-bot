...
const PHRASES = [
    "🦵 {s} delivers a devastating roundhouse kick to {t}!",
    "💥 {s} kicks {t} straight into orbit!",
    "😠 {s} forcefully kicks {t} out of the way!"
];
...
module.exports = {
    ...
    data: new SlashCommandBuilder()
        .setName('kik')
        .setDescription('Use kik on someone! Usage: -kik @user')
    ...
    async execute(interaction, client, args) {
        ...
        try {
            const res = await axios.get('https://nekos.best/api/v2/kik');
            const gif = res.data.results[0];

            const container = new ContainerBuilder()
                .setAccentColor(0xff6b8a)
                .addTextDisplayComponents(
                    new TextDisplayBuilder().setContent('# 🦶 Kik!')
                )
