...
const PHRASES = [
    "🦵 {s} gently guides {t}'s head onto their lap. So comfy~",
    "✨ {t} falls asleep peacefully on {s}'s lap pillow.",
    "💖 {s} gives {t} a warm lap pillow and strokes their hair."
];
...
module.exports = {
    ...
    data: new SlashCommandBuilder()
        .setName('lappillow')
        .setDescription('Use lappillow on someone! Usage: -lappillow @user')
    ...
    async execute(interaction, client, args) {
        ...
        try {
            const res = await axios.get('https://nekos.best/api/v2/lappillow');
            const gif = res.data.results[0];

            const container = new ContainerBuilder()
                .setAccentColor(0xff6b8a)
                .addTextDisplayComponents(
                    new TextDisplayBuilder().setContent('# 💤 Lappillow!')
                )
