...
const PHRASES = [
    "🎉 {s} is bouncing around with sheer happiness!",
    "✨ {s} is practically glowing with joy!",
    "🌈 {s}'s day has been made. So happy!"
];
...
module.exports = {
    ...
    data: new SlashCommandBuilder()
        .setName('happy')
        .setDescription('Express yourself! Usage: -happy')
    ...
    async execute(interaction, client, args) {
        ...
        try {
            const res = await axios.get('https://nekos.best/api/v2/happy');
            const gif = res.data.results[0];

            const container = new ContainerBuilder()
                .setAccentColor(0xff6b8a)
                .addTextDisplayComponents(
                    new TextDisplayBuilder().setContent('# 😄 Happy!')
                )
