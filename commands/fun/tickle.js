...
const PHRASES = [
    "👐 {s} mercilessly tickles {t}! No escape!",
    "😂 {t} bursts into laughter as {s} tickles their sides!",
    "✨ {s} sneaks up and starts a surprise tickle attack on {t}!"
];
...
module.exports = {
    ...
    data: new SlashCommandBuilder()
        .setName('tickle')
        .setDescription('Use tickle on someone! Usage: -tickle @user')
    ...
    async execute(interaction, client, args) {
        ...
        try {
            const res = await axios.get('https://nekos.best/api/v2/tickle');
            const gif = res.data.results[0];

            const container = new ContainerBuilder()
                .setAccentColor(0xff6b8a)
                .addTextDisplayComponents(
                    new TextDisplayBuilder().setContent('# 👉 Tickle!')
                )
