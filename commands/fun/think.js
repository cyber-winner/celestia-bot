...
const PHRASES = [
    "🤔 {s} is lost in deep thought. Do not disturb.",
    "💭 {s} is calculating the mysteries of the universe...",
    "✨ {s} rubs their chin thoughtfully."
];
...
module.exports = {
    ...
    data: new SlashCommandBuilder()
        .setName('think')
        .setDescription('Express yourself! Usage: -think')
    ...
    async execute(interaction, client, args) {
        ...
        try {
            const res = await axios.get('https://nekos.best/api/v2/think');
            const gif = res.data.results[0];

            const container = new ContainerBuilder()
                .setAccentColor(0xff6b8a)
                .addTextDisplayComponents(
                    new TextDisplayBuilder().setContent('# 🤔 Think!')
                )
