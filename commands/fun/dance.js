...
const PHRASES = [
    "💃 {s} is busting out some incredible dance moves!",
    "🕺 {s} hits the dance floor and completely steals the show!",
    "🎶 {s} grooves to the music. What a legend!"
];
...
module.exports = {
    ...
    data: new SlashCommandBuilder()
        .setName('dance')
        .setDescription('Express yourself! Usage: -dance')
    ...
    async execute(interaction, client, args) {
        ...
        try {
            const res = await axios.get('https://nekos.best/api/v2/dance');
            const gif = res.data.results[0];

            const container = new ContainerBuilder()
                .setAccentColor(0xff6b8a)
                .addTextDisplayComponents(
                    new TextDisplayBuilder().setContent('# 💃 Dance!')
                )
