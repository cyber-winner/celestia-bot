...
const PHRASES = [
    "✅ {s} nods in absolute agreement.",
    "✨ {s} gives a firm, approving nod.",
    "👍 {s} nods slowly, understanding the situation completely."
];
...
module.exports = {
    ...
    data: new SlashCommandBuilder()
        .setName('nod')
        .setDescription('Express yourself! Usage: -nod')
    ...
    async execute(interaction, client, args) {
        ...
        try {
            const res = await axios.get('https://nekos.best/api/v2/nod');
            const gif = res.data.results[0];

            const container = new ContainerBuilder()
                .setAccentColor(0xff6b8a)
                .addTextDisplayComponents(
                    new TextDisplayBuilder().setContent('# 👍 Nod!')
                )
