...
const PHRASES = [
    "💋 {s} gives {t} a soft little peck on the cheek!",
    "😚 {s} leans in and quickly pecks {t}!",
    "✨ {s} surprises {t} with a sweet little peck!"
];
...
module.exports = {
    ...
    data: new SlashCommandBuilder()
        .setName('peck')
        .setDescription('Use peck on someone! Usage: -peck @user')
    ...
    async execute(interaction, client, args) {
        ...
        try {
            const res = await axios.get('https://nekos.best/api/v2/peck');
            const gif = res.data.results[0];

            const container = new ContainerBuilder()
                .setAccentColor(0xff6b8a)
                .addTextDisplayComponents(
                    new TextDisplayBuilder().setContent('# 😘 Peck!')
                )
