...
const PHRASES = [
    "💤 {s} couldn't stay awake any longer. Sweet dreams! Zzz...",
    "🛏️ {s} snuggles into a warm blanket and drifts off to sleep.",
    "😴 The exhaustion finally caught up! {s} has fallen asleep."
];
...
module.exports = {
    ...
    data: new SlashCommandBuilder()
        .setName('sleep')
        .setDescription('Express yourself! Usage: -sleep')
    ...
    async execute(interaction, client, args) {
        ...
        try {
            const res = await axios.get('https://nekos.best/api/v2/sleep');
            const gif = res.data.results[0];

            const container = new ContainerBuilder()
                .setAccentColor(0xff6b8a)
                .addTextDisplayComponents(
                    new TextDisplayBuilder().setContent('# 😴 Sleep!')
                )
