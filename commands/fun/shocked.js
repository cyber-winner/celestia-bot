...
const PHRASES = [
    "😱 {s} gasps in pure, unadulterated shock!",
    "⚡ {s} is absolutely flabbergasted!",
    "🙀 {s}'s jaw just dropped straight to the floor."
];
...
module.exports = {
    ...
    data: new SlashCommandBuilder()
        .setName('shocked')
        .setDescription('Express yourself! Usage: -shocked')
    ...
    async execute(interaction, client, args) {
        ...
        try {
            const res = await axios.get('https://nekos.best/api/v2/shocked');
            const gif = res.data.results[0];

            const container = new ContainerBuilder()
                .setAccentColor(0xff6b8a)
                .addTextDisplayComponents(
                    new TextDisplayBuilder().setContent('# 😱 Shocked!')
                )
