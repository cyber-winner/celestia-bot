...
const PHRASES = [
    "💪 {s} effortlessly lifts {t} up and carries them away!",
    "✨ {s} gives {t} a romantic bridal carry!",
    "🎒 {s} throws {t} over their shoulder like a sack of potatoes!"
];
...
module.exports = {
    ...
    data: new SlashCommandBuilder()
        .setName('carry')
        .setDescription('Use carry on someone! Usage: -carry @user')
    ...
    async execute(interaction, client, args) {
        ...
        try {
            const res = await axios.get('https://nekos.best/api/v2/carry');
            const gif = res.data.results[0];

            const container = new ContainerBuilder()
                .setAccentColor(0xff6b8a)
                .addTextDisplayComponents(
                    new TextDisplayBuilder().setContent('# 🎒 Carry!')
                )
