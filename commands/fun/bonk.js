...
const PHRASES = [
    "🔨 {s} aggressively BONKS {t} on the head! Go to horny jail!",
    "💥 *THWACK!* {s} just gave {t} a massive bonk!",
    "💢 {s} smacks {t} with a giant squeaky hammer. BONK!"
];
...
module.exports = {
    ...
    data: new SlashCommandBuilder()
        .setName('bonk')
        .setDescription('Use bonk on someone! Usage: -bonk @user')
    ...
    async execute(interaction, client, args) {
        ...
        try {
            const res = await axios.get('https://nekos.best/api/v2/bonk');
            const gif = res.data.results[0];

            const container = new ContainerBuilder()
                .setAccentColor(0xff6b8a)
                .addTextDisplayComponents(
                    new TextDisplayBuilder().setContent('# 🔨 Bonk!')
                )
