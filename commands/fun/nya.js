...
const PHRASES = [
    "🐱 {s} curls up and goes NYA~",
    "🐾 {s} embraces their inner catgirl/catboy. Nyaaa!",
    "✨ Meow! {s} is feeling very nya today!"
];
...
module.exports = {
    ...
    data: new SlashCommandBuilder()
        .setName('nya')
        .setDescription('Express yourself! Usage: -nya')
    ...
    async execute(interaction, client, args) {
        ...
        try {
            const res = await axios.get('https://nekos.best/api/v2/nya');
            const gif = res.data.results[0];

            const container = new ContainerBuilder()
                .setAccentColor(0xff6b8a)
                .addTextDisplayComponents(
                    new TextDisplayBuilder().setContent('# 🐾 Nya!')
                )
