...
const PHRASES = [
    "😤 {s} crosses their arms and pouts. Hmph!",
    "😠 {s} is visibly pouting... give them what they want!",
    "✨ {s} puffs out their cheeks in a cute little pout."
];
...
module.exports = {
    ...
    data: new SlashCommandBuilder()
        .setName('pout')
        .setDescription('Express yourself! Usage: -pout')
    ...
    async execute(interaction, client, args) {
        ...
        try {
            const res = await axios.get('https://nekos.best/api/v2/pout');
            const gif = res.data.results[0];

            const container = new ContainerBuilder()
                .setAccentColor(0xff6b8a)
                .addTextDisplayComponents(
                    new TextDisplayBuilder().setContent('# 😤 Pout!')
                )
