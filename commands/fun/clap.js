...
const PHRASES = [
    "👏 {s} is giving a roaring round of applause!",
    "🎊 {s} claps enthusiastically! Bravo! Bravo!",
    "✨ {s} stands up and claps in pure admiration."
];
...
module.exports = {
    ...
    data: new SlashCommandBuilder()
        .setName('clap')
        .setDescription('Express yourself! Usage: -clap')
    ...
    async execute(interaction, client, args) {
        ...
        try {
            const res = await axios.get('https://nekos.best/api/v2/clap');
            const gif = res.data.results[0];

            const container = new ContainerBuilder()
                .setAccentColor(0xff6b8a)
                .addTextDisplayComponents(
                    new TextDisplayBuilder().setContent('# 👏 Clap!')
                )
