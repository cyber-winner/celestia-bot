...
const PHRASES = [
    "💢 {s} yells at {t}: You're such a BAKA!",
    "😤 {s} turns away, pouting. B-baka {t}...",
    "👿 {s} scolds {t} for being an absolute idiot. BAKA!"
];
...
module.exports = {
    ...
    data: new SlashCommandBuilder()
        .setName('baka')
        .setDescription('Use baka on someone! Usage: -baka @user')
    ...
    async execute(interaction, client, args) {
        ...
        try {
            const res = await axios.get('https://nekos.best/api/v2/baka');
            const gif = res.data.results[0];

            const container = new ContainerBuilder()
                .setAccentColor(0xff6b8a)
                .addTextDisplayComponents(
                    new TextDisplayBuilder().setContent('# 💢 Baka!')
                )
