...
const PHRASES = [
    "😛 {s} sticks their tongue out! BLEH!",
    "😝 {s} goes completely bleh. How childish~",
    "😜 {s} teases everyone with a loud BLEEEH!"
];
...
module.exports = {
    ...
    data: new SlashCommandBuilder()
        .setName('bleh')
        .setDescription('Express yourself! Usage: -bleh')
    ...
    async execute(interaction, client, args) {
        ...
        try {
            const res = await axios.get('https://nekos.best/api/v2/bleh');
            const gif = res.data.results[0];

            const container = new ContainerBuilder()
                .setAccentColor(0xff6b8a)
                .addTextDisplayComponents(
                    new TextDisplayBuilder().setContent('# 😜 Bleh!')
                )
