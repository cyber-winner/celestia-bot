...
const PHRASES = [
    "☕ {s} takes a slow, dramatic sip of their drink.",
    "🍵 {s} is just sitting here, sipping quietly...",
    "🧃 {s} aggressively sips their juice box."
];
...
module.exports = {
    ...
    data: new SlashCommandBuilder()
        .setName('sip')
        .setDescription('Express yourself! Usage: -sip')
    ...
    async execute(interaction, client, args) {
        ...
        try {
            const res = await axios.get('https://nekos.best/api/v2/sip');
            const gif = res.data.results[0];

            const container = new ContainerBuilder()
                .setAccentColor(0xff6b8a)
                .addTextDisplayComponents(
                    new TextDisplayBuilder().setContent('# 🥤 Sip!')
                )
