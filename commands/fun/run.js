...
const PHRASES = [
    "🏃 {s} is sprinting away at the speed of sound!",
    "💨 {s} yells 'NOPE!' and runs for their life!",
    "🏁 {s} dashes off into the sunset."
];
...
module.exports = {
    ...
    data: new SlashCommandBuilder()
        .setName('run')
        .setDescription('Express yourself! Usage: -run')
    ...
    async execute(interaction, client, args) {
        ...
        try {
            const res = await axios.get('https://nekos.best/api/v2/run');
            const gif = res.data.results[0];

            const container = new ContainerBuilder()
                .setAccentColor(0xff6b8a)
                .addTextDisplayComponents(
                    new TextDisplayBuilder().setContent('# 🏃 Run!')
                )
