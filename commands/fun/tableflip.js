...
const PHRASES = [
    "┻━┻ ︵ヽ(Д´)ﾉ︵ ┻━┻ {s} FLIPS THE FREAKING TABLE!",
    "💥 {s} had enough and flipped everything over!",
    "💢 {s} dramatically throws the table into the sun!"
];
...
module.exports = {
    ...
    data: new SlashCommandBuilder()
        .setName('tableflip')
        .setDescription('Express yourself! Usage: -tableflip')
    ...
    async execute(interaction, client, args) {
        ...
        try {
            const res = await axios.get('https://nekos.best/api/v2/tableflip');
            const gif = res.data.results[0];

            const container = new ContainerBuilder()
                .setAccentColor(0xff6b8a)
                .addTextDisplayComponents(
                    new TextDisplayBuilder().setContent('# ┬─┬ノ( º _ ºノ) Tableflip!')
                )
