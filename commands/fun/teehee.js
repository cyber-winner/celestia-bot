...
const PHRASES = [
    "🤭 {s} giggles mischieviously... Teehee!",
    "✨ {s} covers their mouth and lets out a soft teehee~",
    "😈 {s} is definitely planning something. *Teehee*"
];
...
module.exports = {
    ...
    data: new SlashCommandBuilder()
        .setName('teehee')
        .setDescription('Express yourself! Usage: -teehee')
    ...
    async execute(interaction, client, args) {
        ...
        try {
            const res = await axios.get('https://nekos.best/api/v2/teehee');
            const gif = res.data.results[0];

            const container = new ContainerBuilder()
                .setAccentColor(0xff6b8a)
                .addTextDisplayComponents(
                    new TextDisplayBuilder().setContent('# 🤭 Teehee!')
                )
