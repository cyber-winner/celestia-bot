...
const PHRASES = [
    "❌ {s} firmly says NOPE. Absolutely not.",
    "🛑 {s} crosses their arms. That's a big NOPE from me.",
    "🚶 {s} turns around and walks away. Nope."
];
...
module.exports = {
    ...
    data: new SlashCommandBuilder()
        .setName('nope')
        .setDescription('Express yourself! Usage: -nope')
    ...
    async execute(interaction, client, args) {
        ...
        try {
            const res = await axios.get('https://nekos.best/api/v2/nope');
            const gif = res.data.results[0];

            const container = new ContainerBuilder()
                .setAccentColor(0xff6b8a)
                .addTextDisplayComponents(
                    new TextDisplayBuilder().setContent('# ❌ Nope!')
                )
