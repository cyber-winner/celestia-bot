...
const PHRASES = [
    "🍔 {s} is happily nomming on a giant meal! NOM NOM!",
    "🍩 {s} takes a huge bite. Nom nom nom~",
    "✨ {s} is quietly munching away in the corner."
];
...
module.exports = {
    ...
    data: new SlashCommandBuilder()
        .setName('nom')
        .setDescription('Express yourself! Usage: -nom')
    ...
    async execute(interaction, client, args) {
        ...
        try {
            const res = await axios.get('https://nekos.best/api/v2/nom');
            const gif = res.data.results[0];

            const container = new ContainerBuilder()
                .setAccentColor(0xff6b8a)
                .addTextDisplayComponents(
                    new TextDisplayBuilder().setContent('# 😋 Nom!')
                )
