...
const PHRASES = [
    "🥱 {s} lets out a massive yawn. So sleepy...",
    "💤 {s} rubs their eyes and yawns tiredly.",
    "✨ {s} stretches their arms and yawns."
];
...
module.exports = {
    ...
    data: new SlashCommandBuilder()
        .setName('yawn')
        .setDescription('Express yourself! Usage: -yawn')
    ...
    async execute(interaction, client, args) {
        ...
        try {
            const res = await axios.get('https://nekos.best/api/v2/yawn');
            const gif = res.data.results[0];

            const container = new ContainerBuilder()
                .setAccentColor(0xff6b8a)
                .addTextDisplayComponents(
                    new TextDisplayBuilder().setContent('# 🥱 Yawn!')
                )
