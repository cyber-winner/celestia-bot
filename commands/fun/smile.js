...
const PHRASES = [
    "😊 {s} flashes a bright, heartwarming smile!",
    "✨ {s} smiles gently, filling the room with warmth.",
    "😇 A pure, innocent smile appears on {s}'s face!"
];
...
module.exports = {
    ...
    data: new SlashCommandBuilder()
        .setName('smile')
        .setDescription('Express yourself! Usage: -smile')
    ...
    async execute(interaction, client, args) {
        ...
        try {
            const res = await axios.get('https://nekos.best/api/v2/smile');
            const gif = res.data.results[0];

            const container = new ContainerBuilder()
                .setAccentColor(0xff6b8a)
                .addTextDisplayComponents(
                    new TextDisplayBuilder().setContent('# 😊 Smile!')
                )
