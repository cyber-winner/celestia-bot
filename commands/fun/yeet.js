...
const PHRASES = [
    "🚀 {s} grabs {t} and YEETS them into the stratosphere!",
    "☄️ {t} was successfully yeeted out of the group by {s}!",
    "💨 {s} shouts 'YEET!' and throws {t} across the room!"
];
...
module.exports = {
    ...
    data: new SlashCommandBuilder()
        .setName('yeet')
        .setDescription('Use yeet on someone! Usage: -yeet @user')
    ...
    async execute(interaction, client, args) {
        ...
        try {
            const res = await axios.get('https://nekos.best/api/v2/yeet');
            const gif = res.data.results[0];

            const container = new ContainerBuilder()
                .setAccentColor(0xff6b8a)
                .addTextDisplayComponents(
                    new TextDisplayBuilder().setContent('# 🚀 Yeet!')
                )
