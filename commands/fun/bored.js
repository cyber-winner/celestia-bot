...
const PHRASES = [
    "🥱 {s} is dying of boredom here...",
    "😑 {s} stares blankly at the wall. This is so boring.",
    "✨ {s} sighs loudly. Entertain me!"
];
...
module.exports = {
    ...
    data: new SlashCommandBuilder()
        .setName('bored')
        .setDescription('Express yourself! Usage: -bored')
    ...
    async execute(interaction, client, args) {
        ...
        try {
            const res = await axios.get('https://nekos.best/api/v2/bored');
            const gif = res.data.results[0];

            const container = new ContainerBuilder()
                .setAccentColor(0xff6b8a)
                .addTextDisplayComponents(
                    new TextDisplayBuilder().setContent('# 🥱 Bored!')
                )
