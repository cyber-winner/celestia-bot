...
const PHRASES = [
    "🤝 {s} and {t} firmly shake hands. It's a deal!",
    "✨ {s} shakes {t}'s hand respectfully.",
    "🤝 {s} locks eyes with {t} during an intense handshake."
];
...
module.exports = {
    ...
    data: new SlashCommandBuilder()
        .setName('handshake')
        .setDescription('Use handshake on someone! Usage: -handshake @user')
    ...
    async execute(interaction, client, args) {
        ...
        try {
            const res = await axios.get('https://nekos.best/api/v2/handshake');
            const gif = res.data.results[0];

            const container = new ContainerBuilder()
                .setAccentColor(0xff6b8a)
                .addTextDisplayComponents(
                    new TextDisplayBuilder().setContent('# 🤝 Handshake!')
                )
