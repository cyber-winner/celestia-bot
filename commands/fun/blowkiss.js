...
const PHRASES = [
    "😘 {s} blows a charming kiss right to {t}!",
    "💕 {s} sends a magical flying kiss towards {t}!",
    "✨ {s} blows a kiss... {t} catches it!"
];
...
module.exports = {
    ...
    data: new SlashCommandBuilder()
        .setName('blowkiss')
        .setDescription('Use blowkiss on someone! Usage: -blowkiss @user')
    ...
    async execute(interaction, client, args) {
        ...
        try {
            const res = await axios.get('https://nekos.best/api/v2/blowkiss');
            const gif = res.data.results[0];

            const container = new ContainerBuilder()
                .setAccentColor(0xff6b8a)
                .addTextDisplayComponents(
                    new TextDisplayBuilder().setContent('# 😘 Blowkiss!')
                )
