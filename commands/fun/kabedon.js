...
const PHRASES = [
    "💥 {s} aggressively slams the wall next to {t}'s head! KABEDON!",
    "😳 {s} traps {t} against the wall. The tension is real!",
    "✨ {s} gazes deeply into {t}'s eyes after a smooth kabedon."
];
...
module.exports = {
    ...
    data: new SlashCommandBuilder()
        .setName('kabedon')
        .setDescription('Use kabedon on someone! Usage: -kabedon @user')
    ...
    async execute(interaction, client, args) {
        ...
        try {
            const res = await axios.get('https://nekos.best/api/v2/kabedon');
            const gif = res.data.results[0];

            const container = new ContainerBuilder()
                .setAccentColor(0xff6b8a)
                .addTextDisplayComponents(
                    new TextDisplayBuilder().setContent('# 🚪 Kabedon!')
                )
