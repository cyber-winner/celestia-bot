...
const PHRASES = [
    "😵‍💫 {s} is incredibly confused right now. What is happening?!",
    "❓ {s} tilts their head in utter confusion.",
    "🌀 {s}'s brain just short-circuited. Error 404: Logic not found."
];
...
module.exports = {
    ...
    data: new SlashCommandBuilder()
        .setName('confused')
        .setDescription('Express yourself! Usage: -confused')
    ...
    async execute(interaction, client, args) {
        ...
        try {
            const res = await axios.get('https://nekos.best/api/v2/confused');
            const gif = res.data.results[0];

            const container = new ContainerBuilder()
                .setAccentColor(0xff6b8a)
                .addTextDisplayComponents(
                    new TextDisplayBuilder().setContent('# 😕 Confused!')
                )
