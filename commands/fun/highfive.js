...
const PHRASES = [
    "🙌 {s} gives {t} an epic high-five! TEAMWORK!",
    "✨ {s} and {t} share a satisfying, perfectly timed high-five!",
    "👏 {s} leaves {t} hanging... wait, no, they finally high-fived!"
];
...
module.exports = {
    ...
    data: new SlashCommandBuilder()
        .setName('highfive')
        .setDescription('Use highfive on someone! Usage: -highfive @user')
    ...
    async execute(interaction, client, args) {
        ...
        try {
            const res = await axios.get('https://nekos.best/api/v2/highfive');
            const gif = res.data.results[0];

            const container = new ContainerBuilder()
                .setAccentColor(0xff6b8a)
                .addTextDisplayComponents(
                    new TextDisplayBuilder().setContent('# 🙌 Highfive!')
                )
