...
const PHRASES = [
    "🔫 {s} unleashes a barrage of bullets at {t}! PEW PEW PEW!",
    "💥 {s} perfectly snipes {t} from across the map! HEADSHOT!",
    "🎯 {t} didn't even see it coming... {s} just blasted them away!"
];
...
module.exports = {
    ...
    data: new SlashCommandBuilder()
        .setName('shoot')
        .setDescription('Use shoot on someone! Usage: -shoot @user')
    ...
    async execute(interaction, client, args) {
        ...
        try {
            const res = await axios.get('https://nekos.best/api/v2/shoot');
            const gif = res.data.results[0];

            const container = new ContainerBuilder()
                .setAccentColor(0xff6b8a)
                .addTextDisplayComponents(
                    new TextDisplayBuilder().setContent('# 🔫 Shoot!')
                )
