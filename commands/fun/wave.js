...
const PHRASES = [
    "👋 {s} waves energetically! Hello there!",
    "✨ {s} gives a cute little wave to everyone.",
    "🤗 {s} waves with a big, bright smile!"
];
...
module.exports = {
    ...
    data: new SlashCommandBuilder()
        .setName('wave')
        .setDescription('Express yourself! Usage: -wave')
    ...
    async execute(interaction, client, args) {
        ...
        try {
            const res = await axios.get('https://nekos.best/api/v2/wave');
            const gif = res.data.results[0];

            const container = new ContainerBuilder()
                .setAccentColor(0xff6b8a)
                .addTextDisplayComponents(
                    new TextDisplayBuilder().setContent('# 👋 Wave!')
                )
