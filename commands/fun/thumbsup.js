...
const PHRASES = [
    "👍 {s} gives a big, supportive thumbs up!",
    "✨ {s} smiles and flashes a thumbs up. You got this!",
    "💯 {s} approves with an enthusiastic thumbs up!"
];
...
module.exports = {
    ...
    data: new SlashCommandBuilder()
        .setName('thumbsup')
        .setDescription('Express yourself! Usage: -thumbsup')
    ...
    async execute(interaction, client, args) {
        ...
        try {
            const res = await axios.get('https://nekos.best/api/v2/thumbsup');
            const gif = res.data.results[0];

            const container = new ContainerBuilder()
                .setAccentColor(0xff6b8a)
                .addTextDisplayComponents(
                    new TextDisplayBuilder().setContent('# 👍 Thumbsup!')
                )
